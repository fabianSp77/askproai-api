/**
 * Medium Load Test - 50 VUs
 * Target sweet spot for voice latency < 500ms
 */
...
export function handleSummary(data) {
    ...
    if (metrics.http_reqs) {
        output += (metrics.http_reqs.values.rate ? metrics.http_reqs.values.rate.toFixed(2) : 'N/A') + ' req/sec\n';
    }

    output += '\nSUCCESS: ';
    if (metrics.success_rate) {
        output += (metrics.success_rate.values.rate * 100).toFixed(2) + '%\n';
    }
