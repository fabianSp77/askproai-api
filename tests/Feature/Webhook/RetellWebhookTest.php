...
// tests/Feature/Webhook/RetellWebhookTest.php

namespace Tests\Feature\Webhook;

use App\Mail\ErrorNotificationMail;
use App\Models\Tenant; // <-- STANDARD-Trait verwenden!
use App\Models\User;
...
class RetellWebhookTest extends TestCase
{
    ...
    private function getSuccessfulCallPayload(string $callIdSuffix = ''): array
    {
        return ['call_id' => 'test_call_'.uniqid($callIdSuffix), 'status' => 'completed', 'phone_number' => '+491701234567', 'duration' => 120, 'transcript' => '...', 'user_sentiment' => 'positive', 'call_successful' => true, '_name' => 'Max Mustermann', '_email' => 'test@example.com', '_datum__termin' => now()->addWeek()->format('Y-m-d'), '_uhrzeit__termin' => '14:30'];
    }

    private function getInfoCallPayload(string $callIdSuffix = ''): array
    {
        return ['call_id' => 'test_call_'.uniqid($callIdSuffix), 'status' => 'completed', 'phone_number' => '+491709876543', 'duration' => 60, 'transcript' => '...', 'user_sentiment' => 'neutral', 'call_successful' => true];
    }

    private function getFailedCallPayload(string $callIdSuffix = ''): array
    {
        return ['call_id' => 'test_call_'.uniqid($callIdSuffix), 'status' => 'failed', 'phone_number' => '+491701112223', 'duration' => 10, 'transcript' => '...', 'user_sentiment' => 'negative', 'call_successful' => false, 'disconnect_reason' => 'CALLER_HUNG_UP'];
    }

    private function getInvalidDataPayload(): array
    {
        return [/* call_id fehlt */ 'status' => 'completed', '_datum__termin' => 'kein-datum', '_uhrzeit__termin' => 'morgen', '_email' => 'keine-email'];
    }
    // --- Ende Payloads ---

    #[Test]
    public function it_processes_successful_webhook_and_triggers_mocked_booking_for_correct_tenant(): void
    {
        Log::spy();
        Mail::fake();
    ...
    #[Test]
    public function it_processes_webhook_without_appointment_data_correctly_for_correct_tenant(): void
    {
        Log::spy();
        Mail::fake();
        $payload = $this->getInfoCallPayload();
        $response = $this->withHeaders(['X-Tenant-Api-Key' => $this->tenant1->api_key])->postJson('/api/webhooks/retell', $payload);
        $response->assertStatus(200)->assertJson(['success' => true]);
        $this->assertDatabaseHas('calls', ['call_id' => $payload['call_id'], 'tenant_id' => $this->tenant1->id, 'name' => null, 'email' => null]);
        Log::assertNotLogged(fn ($l, $m) => str_contains($m, 'Starte Cal.com'));
        Mail::assertNothingSent();
    }

    #[Test]
    public function it_processes_failed_call_webhook_correctly_for_correct_tenant(): void
    {
        Log::spy();
        Mail::fake();
        $payload = $this->getFailedCallPayload();
        $response = $this->withHeaders(['X-Tenant-Api-Key' => $this->tenant1->api_key])->postJson('/api/webhooks/retell', $payload);
        $response->assertStatus(200)->assertJson(['success' => true]);
    ...
    #[Test]
    public function it_rejects_webhook_with_invalid_data_for_tenant(): void
    {
        Log::spy();
        Mail::fake();
        $payload = $this->getInvalidDataPayload();
        $response = $this->withHeaders(['X-Tenant-Api-Key' => $this->tenant1->api_key])->postJson('/api/webhooks/retell', $payload);
    ...
    #[Test]
    public function it_handles_calcom_service_failure_gracefully_for_tenant(): void
    {
        Log::spy();
        Mail::fake();
