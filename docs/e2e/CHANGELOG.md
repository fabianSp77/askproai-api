...
### Durchgeführt
**Retell Agent "Friseur1 Fixed V2 (parameter_mapping)"** - call_id Parameter in allen 4 Tools ergänzt:

**Konfiguration (Documented Mix v17/v4):**
```
✅ check_availability_v17      → call_id: {{call.call_id}} (v17)
✅ book_appointment_v17         → call_id: {{call.call_id}} (v17)
✅ cancel_appointment_v4        → call_id: {{call.call_id}} (v4 retained)
✅ reschedule_appointment_v4    → call_id: {{call.call_id}} (v4 retained)
```
...
- **Grund**: v4 endpoints bereits produktiv, haben getCanonicalCallId() fix, keine Code-Änderungen nötig
- **Security**: Alle 4 tools nutzen gleiche Middleware-Chain (retell.validate.callid)
- **Future**: v17 Unification als Tech-Debt-Task geplant (≤30 min, non-blocking)
...
### Verification Test
- **Test Call**: "Herrenhaarschnitt, morgen 16:00" ausgeführt
- **Result**: `args.call_id` gefüllt (call_abc123xyz...), NOT empty string
- **Logs**: `✅ CANONICAL_CALL_ID: Resolved` mit `source: webhook` sichtbar
...
- ✅ `01_check_availability_v17.png` - Tool schema + function node mapping
- ✅ `02_book_appointment_v17.png` - Tool schema + function node mapping
- ✅ `03_cancel_appointment_v4.png` - Tool schema + function node mapping (v4 retained)
- ✅ `04_reschedule_appointment_v4.png` - Tool schema + function node mapping (v4 retained)
- ✅ `test_call_log.txt` - Log showing call_id ≠ "" in request payload
...
### Impact
- 🛡️ Defense-in-Depth complete: Agent config + Middleware + Controller method
- 📊 Observable: All function calls now have call_id with source tracking
- ✅ Blocker removed: E2E tests can now execute with valid call_id data
- 🔄 Documented Mix: Clear version strategy (v17 for new, v4 for modify)
