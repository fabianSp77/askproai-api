...
// Keep existing functions but update to include call_id where needed
$updatedFunctions = [];

foreach ($llmConfig['general_tools'] as $tool) {
    // Update collect_appointment_data function
    if ($tool['name'] === 'collect_appointment_data' && $tool['type'] === 'custom') {
        // Ensure proper format for custom function
        $tool['description'] = 'Sammle alle Termindaten und buche den Termin. NIEMALS nach Telefonnummer fragen!';
        
        // For custom functions, we need to ensure the parameters are properly formatted
        ...
        // Add call_id to parameters if it's not there
        ...
        // Remove telefonnummer/phone_number if it exists
        unset($tool['parameters']['properties']['telefonnummer']);
        unset($tool['parameters']['properties']['phone_number']);
        
        // Clean up required array
        if (isset($tool['parameters']['required'])) {
            $tool['parameters']['required'] = array_values(array_filter($tool['parameters']['required'], function($req) {
                return $req !== 'telefonnummer' && $req !== 'phone_number';
            }));
        }
    }
    
    // Update check_customer function
    if ($tool['name'] === 'check_customer' && $tool['type'] === 'custom') {
    ...
    if ($tool['name'] === 'cancel_appointment' && $tool['type'] === 'custom') {
    ...
    // Update reschedule_appointment function
    if ($tool['name'] === 'reschedule_appointment' && $tool['type'] === 'custom') {
        // Add call_id if not present
        if (!isset($tool['parameters']['properties']['call_id'])) {
            $tool['parameters']['properties']['call_id'] = [
                'type' => 'string',
...
if (strpos($currentPrompt, 'NIEMALS nach der Telefonnummer fragen') === false) {
    ...
    foreach ($lines as $index => $line) {
        if (stripos($line, 'WICHTIGE ANWEISUNGEN') !== false || stripos($line, 'REGELN') !== false) {
    ...
    if ($insertIndex > 0) {
        array_splice($lines, $insertIndex, 0, "- NIEMALS nach der Telefonnummer fragen - die Telefonnummer ist bereits über {{caller_phone_number}} verfügbar oder verwende call_id in den Funktionen");
...
// Show which functions have call_id
echo "\nFunctions with call_id parameter:\n";
foreach ($updatedFunctions as $func) {
    if (isset($func['parameters']['properties']['call_id'])) {
        echo "- " . $func['name'] . " ✅\n";
    } else {
        echo "- " . $func['name'] . "\n";
    }
...
if ($updateResponse->successful()) {
    ...
    echo "\n❌ Failed to update LLM: " . $updateResponse->status() . "\n";
