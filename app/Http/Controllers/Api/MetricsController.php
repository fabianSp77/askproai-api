...
class MetricsController extends Controller
{
    ...
    public function metrics(Request $request)
    {
        ...
        // Check if using MetricsCollector (Prometheus library) or MetricsCollectorService
        if (method_exists($this->collector, 'renderMetrics')) {
            // Using MetricsCollector with Prometheus library
            return response($this->collector->renderMetrics())
                ->header('Content-Type', 'text/plain; version=0.0.4');
        } else {
            // Using MetricsCollectorService with custom implementation
