...
class StripeCheckoutService
{
    ...
    public function __construct(CommissionCalculator $commissionCalculator = null)
    {
        ...
        $this->commissionCalculator = $commissionCalculator ?: app(CommissionCalculator::class);
    }
    ...
    public function processSuccessfulPayment(string $sessionId): void
    {
        ...
        DB::transaction(function () use ($session, $topupId) {
            ...
            if ($tenant->parent_id && $tenant->tenant_type === 'reseller_customer') {
                $this->commissionCalculator->calculateTopupCommission($topup);
            }
            ...
            event(new BalanceUpdated($tenant, $creditAmount));
