    ...
    public static function infolist(Infolist $infolist): Infolist
    {
        return $infolist
            ->schema([
                ...
                ->schema([
                        ...
                        ->schema([
                            Infolists\Components\TextEntry::make('customer_name')
                                ...
                                ->getStateUsing(function ($record) {
                                    ...
                                    $status = $record->customer ? '' : ' (nicht verknüpft)';
                                    return $name . $status;
                                })
                                ->size('lg')
                                ->weight(FontWeight::Bold),
                            ...
                            Infolists\Components\Grid::make(3)
                                ->schema([
                                    Infolists\Components\TextEntry::make('customer_status')
                                        ->label('Status')
                                        ->getStateUsing(fn ($record) => $record->first_visit ? 'Neukunde' : 'Bestandskunde')
                                        ->badge()
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            Infolists\Components\TextEntry::make('sentiment_analysis')
                                ...
                                ->getStateUsing(function ($record) {
                                    if ($record->mlPrediction) {
                                        $emoji = match($record->mlPrediction->sentiment_label) {
                                            'positive' => '😊',
                                            'negative' => '😔',
                                            'neutral' => '😐',
                                            default => ''
                                        };
                                        return $emoji . ' ' . ucfirst($record->mlPrediction->sentiment_label) . ' (' . $record->mlPrediction->confidence_percentage . ')';
                                    }
                            ...
                            // Summary
                            Infolists\Components\TextEntry::make('summary_text')
                                ->label('Zusammenfassung')
                                ->getStateUsing(function ($record) {
                                    $summary = $record->mlPrediction?->summary
                                    ...
                                    return mb_strlen($summary) > 150 ? mb_substr($summary, 0, 147) . '...' : $summary;
                                })
                                ->placeholder('Keine Zusammenfassung')
                                ->columnSpanFull(),
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            // Appointment link
                            Infolists\Components\TextEntry::make('appointment_link')
                                ->label('Termin')
                                ->getStateUsing(function ($record) {
                                ...
                                ->url(fn ($record) => $record->appointment ? AppointmentResource::getUrl('view', ['record' => $record->appointment]) : null)
                                ->color(fn ($record) => $record->appointment ? 'primary' : 'gray')
                                ->icon(fn ($record) => $record->appointment ? 'heroicon-m-arrow-top-right-on-square' : null),
                        ])
                        ->extraAttributes([
                            'class' => 'h-full',
                        ]),
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            Infolists\Components\TextEntry::make('insurance_company')
                                ->label('Gesellschaft')
                                ->getStateUsing(fn ($record) => 
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            Infolists\Components\TextEntry::make('cost_breakdown_display')
                ...
                // EXPANDABLE DETAILS - Technical information
                Infolists\Components\Section::make('Erweiterte Details')
                    ...
                    ->schema([
                        ...
                        // Raw data viewers
                        Infolists\Components\KeyValueEntry::make('metadata.customer_data')
                            ->label('Extrahierte Kundendaten (Raw)')
                            ->getStateUsing(function ($record) {
                                $state = $record->metadata['customer_data'] ?? [];
                                if (!is_array($state)) return [];
                                return $state;
                            })
                            ->visible(fn ($record) => !empty($record->metadata['customer_data'])),
                    ]),
            ]);
    }
