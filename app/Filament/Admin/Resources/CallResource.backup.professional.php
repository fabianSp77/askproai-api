...
';
    ...
    public static function infolist(Infolist $infolist): Infolist
    {
        return $infolist
            ->schema([
                // 1. EXECUTIVE SUMMARY CARD - Full width premium header
                Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            Infolists\Components\Group::make([
                                ...
                                Infolists\Components\TextEntry::make('customer_display')
                                    ...
                                    ->getStateUsing(function ($record) {
                                        ...
                                        $company = $record->metadata['customer_data']['company_name'] 
                                ...
                                Infolists\Components\Grid::make(3)
                                    ->schema([
                                        Infolists\Components\TextEntry::make('duration_display')
                                            ...
                                            ->getStateUsing(fn ($record) => 
                                                $record->duration_sec 
                                                    ? gmdate('i:s', $record->duration_sec) . ' Min'
                                                    : '—'
                                            )
                                            ->icon('heroicon-m-clock')
                                            ->size('lg'),
                            ...
                            Infolists\Components\Group::make([
                                Infolists\Components\Grid::make(3)
                                    ->schema([
                                        // Sentiment
                                        Infolists\Components\TextEntry::make('sentiment_badge')
                                            ->label('Stimmung')
                                            ->getStateUsing(function ($record) {
                                        ...
                                        // Urgency
                                        Infolists\Components\TextEntry::make('urgency_badge')
                                            ->label('Dringlichkeit')
                                            ->getStateUsing(function ($record) {
                                            ...
                                            ->size('lg')
                                            ->color(fn ($state) => match(true) {
                                                str_contains($state, 'Dringend') => 'danger',
                                                str_contains($state, 'Mittel') => 'warning',
                                                str_contains($state, 'Niedrig') => 'success',
                                                default => 'gray'
                                            }),
                                            
                                        // Success
                                        Infolists\Components\TextEntry::make('success_badge')
                                            ->label('Status')
                                            ->getStateUsing(function ($record) {
                                                if ($record->call_successful === true) {
                                                    return '✅ Erfolgreich';
                                                } elseif ($record->call_successful === false) {
                                                    return '❌ Fehlgeschlagen';
                                                }
                                                return match($record->call_status) {
                                                    'completed' => '✅ Abgeschlossen',
                                                    'in_progress' => '🔄 Läuft',
                                                    'failed' => '❌ Fehler',
                                                    default => '📞 ' . ucfirst($record->call_status ?? 'Unbekannt')
                                                };
                                            })
                                            ->badge()
                                            ->size('lg')
                                            ->color(fn ($state) => match(true) {
                                                str_contains($state, 'Erfolgreich') || str_contains($state, 'Abgeschlossen') => 'success',
                                                str_contains($state, 'Fehlgeschlagen') || str_contains($state, 'Fehler') => 'danger',
                                                str_contains($state, 'Läuft') => 'warning',
                                                default => 'info'
                                            }),
                                    ]),
                            ])
                    ...
                    ->extraAttributes([
                        'class' => 'bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 shadow-lg rounded-xl',
                        'style' => 'padding: 1.5rem; margin-bottom: 2rem;'
                    ]),
                    
                // 2. PRIMARY FOCUS AREA - Reason and Audio
                Infolists\Components\Grid::make([
                    'default' => 1,
                    'md' => 2,
                    'lg' => 2,
                ...
                ->schema([
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            Infolists\Components\TextEntry::make('appointment_info')
                                ->label('Terminwunsch')
                                ->getStateUsing(function ($record) {
                                    if (!$record->appointment_requested) return null;
                                    
                                    $info = [];
                                    if ($record->appointment_made) {
                                        $info[] = '✅ Termin gebucht';
                                    } else {
                                        $info[] = '📅 Termin angefragt';
                                    }
                                ...
                                ->visible(fn ($record) => $record->appointment_requested)
                                ->badge()
                                ->color('primary'),
                        ])
                        ->extraAttributes([
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            Infolists\Components\TextEntry::make('no_audio')
                                ->label(false)
                                ->state('Keine Aufzeichnung verfügbar')
                                ->visible(fn ($record) => empty($record->audio_url) && empty($record->recording_url)),
                        ])
                        ->extraAttributes([
                            'class' => 'h-full'
                ...
                // 3. INFORMATION DASHBOARD - All visible cards
                Infolists\Components\Grid::make([
                    'default' => 1,
                    'sm' => 2,
                    'lg' => 3,
                ...
                ->schema([
                        ...
                        ->schema([
                            ...
                            Infolists\Components\Group::make([
                                Infolists\Components\TextEntry::make('customer_type')
                                    ...
                                    ->badge()
                                    ->color(fn ($record) => $record->first_visit ? 'info' : 'success'),
                                    
                                Infolists\Components\TextEntry::make('no_shows')
                                    ->label(false)
                                    ->getStateUsing(fn ($record) => 
                                        $record->no_show_count > 0 
                                            ? "⚠️ {$record->no_show_count} No-Shows" 
                                            : null
                                    )
                                    ->badge()
                                    ->color('warning')
                                    ->visible(fn ($record) => $record->no_show_count > 0),
                            ])
                            ->columnSpanFull(),
                        ])
                        ->footerActions([
                            ...
                            Infolists\Components\Grid::make(2)
                                ->schema([
                                    ...
                                    Infolists\Components\TextEntry::make('appointment_made')
                                        ->label('Gebucht')
                                        ->getStateUsing(fn ($record) => $record->appointment_made ? '✅ Ja' : '❌ Nein')
                                        ->badge()
                                        ->color(fn ($record) => $record->appointment_made ? 'success' : 'gray'),
                                ]),
                                
                            Infolists\Components\TextEntry::make('appointment_datetime')
                                ->label('Termin')
                                ->getStateUsing(fn ($record) => 
                                    $record->appointment 
                                ...
                                ->icon('heroicon-m-clock')
                                ->iconColor(fn ($record) => $record->appointment ? 'success' : 'gray')
                                ->weight(fn ($record) => $record->appointment ? FontWeight::SemiBold : null),
                                
                            Infolists\Components\TextEntry::make('service_requested')
                                ->label('Service')
                                ->getStateUsing(fn ($record) => 
                                    $record->metadata['customer_data']['service']
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            Infolists\Components\TextEntry::make('sentiment_detail')
                                ->label('Stimmung')
                                ->getStateUsing(function ($record) {
                                    if ($record->mlPrediction) {
                                        return ucfirst($record->mlPrediction->sentiment_label) 
                                            . ' (' . $record->mlPrediction->confidence_percentage . ')';
                                    }
                                    return 'Nicht analysiert';
                                })
                                ->icon(fn ($record) => match($record->mlPrediction?->sentiment_label ?? null) {
                                    'positive' => 'heroicon-m-face-smile',
                                    'negative' => 'heroicon-m-face-frown',
                                    'neutral' => 'heroicon-m-minus-circle',
                                    default => 'heroicon-m-question-mark-circle'
                                })
                                ->iconColor(fn ($record) => $record->mlPrediction?->sentiment_color ?? 'gray'),
                                
                            Infolists\Components\TextEntry::make('intent')
                                ->label('Absicht')
                                ->getStateUsing(fn ($record) => 
                                    $record->analysis['intent'] 
                                    ?? $record->mlPrediction?->intent
                                    ?? null
                                )
                                ->placeholder('Nicht erkannt')
                                ->badge()
                            ...
                            Infolists\Components\TextEntry::make('entities_count')
                                ->label('Erkannte Daten')
                                ->getStateUsing(fn ($record) => 
                                    count($record->analysis['entities'] ?? []) . ' Entitäten'
                                )
                                ->icon('heroicon-m-tag'),
                        ])
                        ->extraAttributes([
                        ...
                        ->visible(fn ($record) => 
                            $record->mlPrediction || !empty($record->analysis)
                        ),
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            Infolists\Components\TextEntry::make('insurance_name')
                                ->label('Gesellschaft')
                                ->getStateUsing(fn ($record) => 
                            ...
                            Infolists\Components\TextEntry::make('insurance_status')
                                ->label('Status')
                                ->getStateUsing(fn ($record) => $record->versicherungsstatus)
                                ->placeholder('—')
                                ->badge()
                                ->color('info'),
                        ])
                        ...
                        ->visible(fn ($record) => 
                            ...
                            $record->insurance_company || 
                            $record->health_insurance_company ||
                            isset($record->metadata['customer_data']['insurance_company'])
                        ),
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            Infolists\Components\TextEntry::make('total_cost')
                                ->label(false)
                                ->getStateUsing(fn ($record) => $record->cost ?? 0)
                                ->money('EUR')
                                ->size('xl')
                                ->weight(FontWeight::Bold),
                                
                            Infolists\Components\TextEntry::make('cost_details')
                                ->label(false)
                                ->getStateUsing(function ($record) {
                                    ...
                                    foreach ($breakdown['product_costs'] as $item) {
                                        $items[] = $item['product'] . ': €' . number_format($item['cost'] / 100, 2);
                                    }
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            ...
                            Infolists\Components\TextEntry::make('agent_info')
                                ->label('AI Agent')
                                ->getStateUsing(fn ($record) => 
                                    ($record->agent_name ?? 'Standard') . 
                                    ($record->agent_version ? ' ' . $record->agent_version : '')
                                )
                                ->icon('heroicon-m-cpu-chip'),
                        ])
                        ->extraAttributes([
                ...
                // 4. ACTION CENTER - Prominent buttons
                Infolists\Components\Section::make()
                    ->heading(false)
                    ->schema([
                    ...
                    ->extraAttributes([
                        'class' => 'bg-gray-50 dark:bg-gray-900 rounded-xl shadow-inner',
                        'style' => 'padding: 1.5rem; margin-bottom: 2rem;'
                    ])
                    ->visible(fn ($record) => 
                ...
                // 5. TRANSCRIPT & DETAILS - Always expanded
                Infolists\Components\Section::make('Gesprächsverlauf')
                    ...
                    ->schema([
                        ...
                        Infolists\Components\ViewEntry::make('transcript')
                            ...
                            ->columnSpanFull()
                            ->visible(fn ($record) => !empty($record->transcript) || !empty($record->transcript_object)),
                            
                        // Summary if no transcript
                        Infolists\Components\TextEntry::make('full_summary')
                            ->label('Zusammenfassung')
                            ->getStateUsing(function ($record) {
                                return $record->mlPrediction?->summary
                                    ?? $record->analysis['call_summary']
                                    ?? $record->summary
                                    ?? null;
                            })
                            ->prose()
                            ->columnSpanFull()
                            ->visible(fn ($record) => 
                                (empty($record->transcript) && empty($record->transcript_object)) &&
                                ($record->mlPrediction?->summary || $record->analysis['call_summary'] || $record->summary)
                            ),
                            
                        // Extracted entities
                        Infolists\Components\KeyValueEntry::make('entities')
                            ...
                            ->getStateUsing(function ($record) {
                                ...
                                foreach ($entities as $key => $value) {
                                    if (!empty($value)) {
                                        $formatted[$labels[$key] ?? ucfirst($key)] = $value;
                                    }
                                }
                ...
                // 6. TECHNICAL DETAILS - Collapsible for debugging
                Infolists\Components\Section::make('Technische Details')
                    ...
                    ->schema([
                        ...
                        // Performance metrics
                        Infolists\Components\KeyValueEntry::make('performance')
                        ...
                        // Debug link
                        Infolists\Components\Actions::make([
                            Infolists\Components\Actions\Action::make('debug_log')
                                ->label('Retell Debug Log öffnen')
                                ->icon('heroicon-m-arrow-top-right-on-square')
                                ->url(fn ($record) => $record->public_log_url)
                                ->openUrlInNewTab()
                                ->visible(fn ($record) => !empty($record->public_log_url)),
