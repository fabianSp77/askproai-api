...
';
    ...
    public static function infolist(Infolist $infolist): Infolist
    {
        return $infolist
            ->schema([
                // PERSONALIZED HEADER - Customer name and interest instead of Call #ID
                Infolists\Components\Group::make([
                    Infolists\Components\TextEntry::make('personalized_title')
                        ->hiddenLabel()
                        ->getStateUsing(function ($record) {
                            ...
                            // Get the main reason/interest
                            $interest = '';
                            if ($record->reason_for_visit) {
                                // Extract first 50 chars of reason
                            ...
                            return $customerName . $interest;
                        })
                        ->size('2xl')
                        ->weight(FontWeight::Bold)
                        ->extraAttributes(['class' => 'text-gray-900 dark:text-white']),
                        
                    Infolists\Components\TextEntry::make('call_reference')
                        ->hiddenLabel()
                        ->getStateUsing(fn ($record) => 
                            'Call #' . $record->id . ' • ' . $record->created_at->format('d. F Y')
                        )
                        ->size('sm')
                        ->color('gray')
                ...
                // EXECUTIVE SUMMARY BOX - Optimized layout with integrated call reason
                Infolists\Components\Section::make()
                    ->schema([
                        // Top row - Contact and Call details
                        ...
                        ->schema([
                            ...
                            Infolists\Components\Group::make([
                                Infolists\Components\TextEntry::make('section_title_contact')
                                    ->hiddenLabel()
                                    ->state('Kontaktinformationen')
                                    ->size('sm')
                                ...
                                Infolists\Components\TextEntry::make('customer_full_info')
                                    ->hiddenLabel()
                                    ->getStateUsing(function ($record) {
                            ...
                            // Right - Call Details
                            Infolists\Components\Group::make([
                                Infolists\Components\TextEntry::make('section_title_details')
                                    ->hiddenLabel()
                                    ->state('Anrufdetails')
                                    ->size('sm')
                                    ->weight(FontWeight::SemiBold)
                                ...
                                Infolists\Components\Grid::make(1)
                                    ->schema([
                                        Infolists\Components\TextEntry::make('time_duration')
                                            ->hiddenLabel()
                                            ->getStateUsing(fn ($record) => 
                                                $record->start_timestamp?->format('H:i \U\h\r') . ' • ' . 
                                                ($record->duration_sec ? gmdate('i:s', $record->duration_sec) . ' Minuten' : '—')
                                            )
                                            ->icon('heroicon-o-clock')
                                            ->size('base'),
                                            
                                        Infolists\Components\TextEntry::make('status_complete')
                                            ->hiddenLabel()
                                            ->getStateUsing(function ($record) {
                                        ...
                                        Infolists\Components\Grid::make(2)
                                            ->schema([
                                                Infolists\Components\TextEntry::make('sentiment_prof')
                                                    ->hiddenLabel()
                                                    ->getStateUsing(function ($record) {
                                                ...
                                                Infolists\Components\TextEntry::make('urgency_prof')
                                                    ->hiddenLabel()
                                                    ->getStateUsing(function ($record) {
                        ...
                        // Divider
                        Infolists\Components\View::make('filament.components.hr')
                            ->view('filament.components.hr'),
                        
                        // Bottom - Call Reason (Prominent)
                        Infolists\Components\Group::make([
                            Infolists\Components\TextEntry::make('reason_label')
                                ->hiddenLabel()
                                ->state('Anrufgrund:')
                                ->size('sm')
                                ->weight(FontWeight::SemiBold)
                            ...
                            Infolists\Components\TextEntry::make('call_reason_prominent')
                                ->hiddenLabel()
                                ->getStateUsing(function ($record) {
                                ...
                                ->size('lg')
                                ->extraAttributes([
                                    'class' => 'mt-2 text-gray-800 dark:text-gray-200 leading-relaxed'
                                ]),
                        ])
                        ->extraAttributes(['class' => 'p-6 pt-4']),
                    ])
                    ->extraAttributes([
                        'class' => 'shadow-sm border border-gray-200 dark:border-gray-700 mb-6'
                    ]),
                    
                // AUDIO PLAYER with improved design
                Infolists\Components\Section::make()
                    ->heading('Audio-Aufzeichnung')
                    ->schema([
                ...
                // TRANSCRIPT - Modern chat design, directly under audio
                Infolists\Components\Section::make('Gesprächsverlauf')
                    ->schema([
                        Infolists\Components\ViewEntry::make('transcript')
                            ->label(false)
                            ->view('filament.infolists.transcript-sentiment-viewer-professional')
                            ->visible(fn ($record) => !empty($record->transcript) || !empty($record->transcript_object)),
                            
                        Infolists\Components\TextEntry::make('no_transcript')
                            ->hiddenLabel()
                            ->state('Kein Transkript verfügbar')
                            ->size('lg')
                            ->color('gray')
                            ->extraAttributes(['class' => 'text-center py-8'])
                            ->visible(fn ($record) => empty($record->transcript) && empty($record->transcript_object)),
                ...
                // INFORMATION CARDS - Modern grid layout
                Infolists\Components\Grid::make([
                    'default' => 1,
                    'sm' => 2,
                    'lg' => 4,
                ...
                ->schema([
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            Infolists\Components\Grid::make(1)
                                ->schema([
                                    Infolists\Components\TextEntry::make('customer_name_card')
                                        ->hiddenLabel()
                                        ->getStateUsing(fn ($record) => 
                                    ...
                                    Infolists\Components\TextEntry::make('customer_type_badge')
                                        ->hiddenLabel()
                                        ->getStateUsing(fn ($record) => 
                                    ...
                                    Infolists\Components\TextEntry::make('no_show_info')
                                        ...
                                        ->color(fn ($record) => $record->no_show_count >= 3 ? 'danger' : 'warning'),
                                ]),
                        ])
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            Infolists\Components\Grid::make(1)
                                ->schema([
                                    Infolists\Components\TextEntry::make('appointment_status_badges')
                                        ->hiddenLabel()
                                        ->getStateUsing(function ($record) {
                                            $status = [];
                                            ...
                                            return implode(' • ', $status) ?: '✗ Kein Termin';
                                        })
                                        ->badge()
                                        ->color(fn ($state) => match(true) {
                                            str_contains($state, '✓') => 'success',
                                    ...
                                    Infolists\Components\TextEntry::make('appointment_datetime_card')
                                        ->hiddenLabel()
                                        ->getStateUsing(fn ($record) => 
                                            $record->appointment 
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            Infolists\Components\Grid::make(1)
                                ->schema([
                                    Infolists\Components\TextEntry::make('sentiment_analysis_card')
                                        ->label('Sentiment')
                                        ->getStateUsing(function ($record) {
                                            if (!$record->mlPrediction) return '— Nicht analysiert';
                                            
                                            $label = ucfirst($record->mlPrediction->sentiment_label);
                                            $confidence = $record->mlPrediction->confidence_percentage;
                                            
                                            return "● $label ($confidence)";
                                        })
                                        ->color(fn ($record) => 
                                            $record->mlPrediction?->sentiment_color ?? 'gray'
                                        ),
                                        
                                    Infolists\Components\TextEntry::make('intent_card')
                                        ->label('Intent')
                                        ->getStateUsing(fn ($record) => 
                                            ucfirst($record->analysis['intent'] ?? 'Unklar')
                                        ),
                                ]),
                        ])
                        ->extraAttributes(['class' => 'h-full'])
                    ...
                    Infolists\Components\Section::make()
                        ...
                        ->schema([
                            Infolists\Components\Grid::make(1)
                                ->schema([
                                    Infolists\Components\TextEntry::make('cost_amount')
                                        ->hiddenLabel()
                                        ->money('EUR')
                                        ->size('xl')
                                        ->weight(FontWeight::Bold)
                                    ...
                                    Infolists\Components\Actions::make([
                                        Infolists\Components\Actions\Action::make('cost_details')
                                            ->label('Details anzeigen')
                                            ->icon('heroicon-m-information-circle')
                                            ->size('sm')
                                            ->modalContent(function ($record) {
                                                if (!$record->cost_breakdown) return 'Keine Details verfügbar';
                                                
                                                $breakdown = is_string($record->cost_breakdown) 
                                                    ? json_decode($record->cost_breakdown, true) 
                                                    : $record->cost_breakdown;
                                                
                                                if (!$breakdown || !isset($breakdown['product_costs'])) {
                                                    return 'Keine Details verfügbar';
                                                }
                                                
                                                $html = '<ul class="space-y-2">';
                                                foreach ($breakdown['product_costs'] as $item) {
                                                    $cost = number_format($item['cost'] / 100, 2);
                                                    $html .= "<li>{$item['product']}: €{$cost}</li>";
                                                }
                                                $html .= '</ul>';
                                                
                                                return new HtmlString($html);
                                            })
                                            ->modalHeading('Kostenaufschlüsselung')
                                            ->modalSubmitAction(false)
                ...
                // ACTION CENTER - If actions available
                Infolists\Components\Section::make()
                    ->heading(false)
                    ->schema([
                        Infolists\Components\Actions::make([
                    ...
                    ->extraAttributes([
                        'class' => 'bg-gray-50 dark:bg-gray-900 rounded-lg p-4 mb-6',
                    ])
                    ->visible(fn ($record) => 
