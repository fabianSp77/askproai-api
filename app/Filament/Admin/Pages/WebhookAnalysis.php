    ...
    public function getMissingDataReport(): Collection
    {
        return Call::latest()
            ...
            ->map(function ($call) {
                ...
                if (empty($call->from_number)) {
                    $missing[] = 'from_number';
                }
                if (empty($call->to_number)) {
                    $missing[] = 'to_number';
                }
                if (empty($call->duration_sec)) {
                    $missing[] = 'duration_sec';
                }
                if (empty($call->transcript)) {
                    $missing[] = 'transcript';
                }
                if (empty($call->branch_id)) {
                    $missing[] = 'branch_id';
                }
                if (empty($call->analysis) || (is_array($call->analysis) && count($call->analysis) === 0)) {
                    $missing[] = 'analysis';
                }
