...
const CallShow = () => {
    ...
    const fetchCallDetails = async () => {
        ...
        try {
            ...
            if (data.call) {
                ...
                setCall(data); // Fallback to entire response
            }
        } catch (err) {
    ...
    const fetchNavigation = async () => {
        ...
        try {
            ...
            const response = await axiosInstance.get(`/calls/${callId}/navigation?`);
    ...
    useEffect(() => {
        const handleKeyPress = (e) => {
            ...
            if (e.key === 'ArrowLeft' && navigation?.previous) {
                navigate(`/calls/${navigation.previous.id}${window.location.search}`);
            } else if (e.key === 'ArrowRight' && navigation?.next) {
                navigate(`/calls/${navigation.next.id}${window.location.search}`);
            }
        };
    ...
    const translateSummary = async (summaryText) => {
        ...
        try {
            ...
            if (response.data) {
                const data = await response.data;
                setTranslatedSummary(data.translated_text || summaryText);
            } else {
    ...
    return (
        <div className="p-6 space-y-6 relative">
            ...
            <div className="bg-gray-50 border-b border-gray-200 -m-6 mb-6 p-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 border-l pl-4">
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => navigation?.previous && navigate(`/calls/${navigation.previous.id}${window.location.search}`)}
                            ...
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => navigation?.next && navigate(`/calls/${navigation.next.id}${window.location.search}`)}
            ...
            {/* Smart Info Cards - 3 Column Layout */}
                    ...
                    <CardContent className="space-y-3">
                        {call.custom_analysis_data?.urgency_level && (
                            <div>
                                ...
                                <Badge variant={
                                    call.custom_analysis_data.urgency_level === 'dringend' || 
                                    call.custom_analysis_data.urgency_level === 'sehr dringend' ? 'destructive' : 
                                    'default'
                                } className="text-sm">
                                    {call.custom_analysis_data.urgency_level.toUpperCase()}
                                </Badge>
                            </div>
                        )}
                        {call.recording_url && (
                    ...
                    <CardContent className="p-0">
                        <Tabs value={activeTab} onValueChange={setActiveTab}>
                            ...
                            <TabsContent value="details" className="p-6 space-y-4">
                                ...
                                {/* Extrahierte Termindaten */}
                                {(call.datum_termin || call.uhrzeit_termin || call.dienstleistung) && (
                                        ...
                                        <div className="grid gap-3 text-sm">
                                            {call.datum_termin && (
                                                <div className="grid grid-cols-3 gap-2">
                                                    ...
                                                    <span className="col-span-2 font-medium">{call.datum_termin}</span>
                                                </div>
                                            )}
                                            {call.uhrzeit_termin && (
                                                <div className="grid grid-cols-3 gap-2">
                                                    <span className="text-muted-foreground">Uhrzeit:</span>
                                                    <span className="col-span-2 font-medium">{call.uhrzeit_termin}</span>
                                                </div>
                                            )}
                                            {call.dienstleistung && (
                                                <div className="grid grid-cols-3 gap-2">
                                                    <span className="text-muted-foreground">Dienstleistung:</span>
                                                    <span className="col-span-2 font-medium">{call.dienstleistung}</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
