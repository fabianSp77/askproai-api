...
<div class="p-4">
    ...
    @if(config('app.debug'))
        <div class="mb-4 p-2 bg-yellow-100 dark:bg-yellow-900 rounded text-xs">
            ...
            <div>Columns: {{ count($columns) }}</div>
            <div>User: {{ auth()->id() }}</div>
        </div>
    @endif

    <div
        x-data="columnManager('{{ $resource }}', {{ auth()->id() }}, @js($columns))"
    >
        {{-- Instructions --}}
        <div class="mb-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>Ziehen Sie die Spalten zum Sortieren</span>
            </div>
            <div class="flex items-center gap-2 mt-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>Checkboxen zum Ein-/Ausblenden</span>
            </div>
        </div>
        ...
        {{-- Column List --}}
        <div x-ref="columnList" class="space-y-2 max-h-[400px] overflow-y-auto pr-2">
            <template x-for="(column, index) in columns" :key="column.key">
                <div
                    :data-column="column.key"
                    class="flex items-center gap-3 p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-primary-300 dark:hover:border-primary-600"
                >
                    {{-- Drag Handle --}}
                    <div class="drag-handle cursor-move p-1">
                    ...
                    <label class="flex-1 flex items-center cursor-pointer">
                        <input
                            ...
                            class="mr-3 h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                        >
                        <span
                    ...
                    {{-- Index --}}
                    <span class="text-xs px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded" x-text="index + 1"></span>
                </div>
            </template>
        </div>
