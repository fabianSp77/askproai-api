<div class="space-y-4">
    @if($record->audio_url ?? $record->recording_url ?? null)
        <audio controls class="w-full" id="modal-audio-{{ $record->id }}">
            <source src="{{ $record->audio_url ?? $record->recording_url }}" type="audio/mpeg">
        ...
        <div class="flex items-center justify-between pt-4 border-t dark:border-gray-700">
            <div class="text-sm text-gray-600 dark:text-gray-400">
                <span class="font-medium">Dauer:</span> {{ gmdate('i:s', $record->duration_sec ?? 0) }} Min.
            </div>
            <a href="{{ $record->audio_url ?? $record->recording_url }}" 
                download="anruf-{{ $record->id }}-{{ ($record->start_timestamp ?? $record->created_at)->format('Y-m-d') }}.mp3" 
        ...
        @if($record->transcript)
            <div class="pt-4 border-t dark:border-gray-700">
                ...
                <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 max-h-64 overflow-y-auto">
                    <pre class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ $record->transcript }}</pre>
                </div>
            </div>
        @endif
    @else
