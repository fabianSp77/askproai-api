...
<div 
    x-data="audioPlayerUltraFixed(@js($audioUrl), @js($sentimentData), @js($transcriptObject), @js($dbDuration), @js($callId))" 
    x-init="init()"
    ...
    <div class="relative bg-white/60 dark:bg-gray-800/60 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/20 p-8 transform transition-all duration-500 hover:shadow-3xl">
            ...
            <div class="flex items-center gap-6" x-show="!loading && audioUrl">
                ...
                <div class="text-right">
                    <div class="flex items-center gap-2">
                        ...
                        <div class="px-4 py-2 bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-full">
                            ...
                            <span class="font-mono text-sm font-semibold" x-text="formatTime(actualDuration || 0)"></span>
                        </div>
                    </div>
                    <div x-show="durationMismatch && dbDuration > 0" class="text-xs text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-1">
                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                        </svg>
                        <span>DB sagt: <span x-text="formatTime(dbDuration)"></span></span>
                    </div>
                </div>
                ...
                <div class="flex flex-col items-center">
                    ...
                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Tatsächlich: <span x-text="Math.round(actualDuration || 0)"></span>s
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-6 mb-8" x-show="audioUrl && !loading">
            ...
            <div class="flex items-center gap-4">
                ...
                <button 
                    @click="skip(-10)"
                    class="p-3 bg-gray-100 dark:bg-gray-700 rounded-full text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-all transform hover:scale-105 active:scale-95"
                    title="10 Sekunden zurück"
                >
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M11.99 5V1l-5 5 5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6h-2c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z"/>
                        <text x="12" y="16" text-anchor="middle" font-size="10" fill="currentColor">10</text>
                    </svg>
                </button>
            ...
            <div class="flex items-center gap-4">
                {{-- Speed Control --}}
                <div class="relative" x-data="{ showSpeed: false }">
                    <button 
                        @click="showSpeed = !showSpeed"
                        class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-all"
                    >
                        <span x-text="playbackRate + 'x'"></span>
                    </button>
                    <div 
                        x-show="showSpeed" 
                        @click.away="showSpeed = false"
                        x-transition
                        class="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-2 space-y-1"
                    >
                        <template x-for="speed in [0.5, 0.75, 1, 1.25, 1.5, 2]">
                            <button 
                                @click="setPlaybackRate(speed); showSpeed = false"
                                class="block w-full px-3 py-1 text-sm rounded hover:bg-gray-100 dark:hover:bg-gray-700"
                                :class="{ 'bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400': playbackRate === speed }"
                                x-text="speed + 'x'"
                            ></button>
                        </template>
                    </div>
                ...
                {{-- Volume Control --}}
                <div class="flex items-center gap-2">
                    <button @click="toggleMute()" class="text-gray-600 dark:text-gray-300">
                        <svg x-show="!muted && volume > 0.5" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 2.929a1 1 0 011.414 0A9.972 9.972 0 0119 10a9.972 9.972 0 01-2.929 7.071 1 1 0 01-1.414-1.414A7.971 7.971 0 0017 10c0-2.21-.894-4.208-2.343-5.657a1 1 0 010-1.414zm-2.829 2.828a1 1 0 011.415 0A5.983 5.983 0 0115 10a5.984 5.984 0 01-1.757 4.243 1 1 0 01-1.415-1.415A3.984 3.984 0 0013 10a3.983 3.983 0 00-1.172-2.828 1 1 0 010-1.415z" clip-rule="evenodd" />
                        </svg>
                        <svg x-show="!muted && volume <= 0.5 && volume > 0" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM12.293 7.293a1 1 0 011.414 0 3 3 0 010 4.243 1 1 0 11-1.414-1.415 1 1 0 000-1.414 1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
                        <svg x-show="muted || volume === 0" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="flex items-center gap-3">
                ...
                <a 
                    :href="audioUrl" 
                    download
                    class="p-3 bg-gray-100 dark:bg-gray-700 rounded-full text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition-all transform hover:scale-105"
                    title="Download"
                >
                ...
                >
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M5 4a1 1 0 00-2 0v2a1 1 0 001 1h2a1 1 0 000-2H5V4zm10 0a1 1 0 012 0v1h1a1 1 0 010 2h-2a1 1 0 01-1-1V4zm-3 11h1v1a1 1 0 102 0v-2a1 1 0 00-1-1h-2a1 1 0 100 2zm-7 0a1 1 0 100 2h2a1 1 0 001-1v-2a1 1 0 10-2 0v1H5z" clip-rule="evenodd" />
                    </svg>
                </button>
            </div>
        </div>
        
        {{-- Advanced Waveform with Timeline --}}
        <div class="mb-8">
            <div id="waveform-{{ $callId }}" class="relative rounded-2xl overflow-hidden shadow-inner bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900"></div>
            
            {{-- Sentiment Timeline Overlay --}}
            <div class="relative h-12 mt-2 bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700 rounded-xl overflow-hidden">
                <div class="absolute inset-0 flex items-center">
                    <template x-for="(segment, index) in sentimentData" :key="index">
                        <div 
                            class="absolute h-8 rounded transition-all duration-300 hover:h-10 hover:-mt-1 cursor-pointer"
                            :class="{
                                'bg-green-400 dark:bg-green-600': segment.sentiment === 'positive',
                                'bg-red-400 dark:bg-red-600': segment.sentiment === 'negative',
                                'bg-gray-300 dark:bg-gray-600': segment.sentiment === 'neutral'
                            }"
                            :style="`left: ${(segment.start_time / actualDuration) * 100}%; width: ${((segment.end_time - segment.start_time) / actualDuration) * 100}%`"
                            :title="`${segment.sentiment} (${segment.score?.toFixed(2) || 'N/A'})`"
                ...
                {{-- Playhead Indicator --}}
                <div 
                    class="absolute top-0 w-0.5 h-full bg-blue-600 dark:bg-blue-400 transition-all duration-100"
            ...
            <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-1">
                <span>0:00</span>
                <span x-text="formatTime(actualDuration / 2)"></span>
                <span x-text="formatTime(actualDuration)"></span>
            </div>
        </div>
        
        {{-- Transcript Sync Display --}}
        <div class="mb-6" x-show="currentSentence">
            <div class="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 rounded-xl">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300" x-text="currentSentence.text"></p>
                        <div class="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                            <span>Sprecher: <span x-text="currentSentence.speaker || 'Unbekannt'"></span></span>
                            <span x-show="currentSentence.sentiment">
                                Stimmung: 
                                <span 
                                    :class="{
                                        'text-green-600 dark:text-green-400': currentSentence.sentiment === 'positive',
                                        'text-red-600 dark:text-red-400': currentSentence.sentiment === 'negative',
                                        'text-gray-600 dark:text-gray-400': currentSentence.sentiment === 'neutral'
                                    }"
                                    x-text="currentSentence.sentiment"
                                ></span>
                            </span>
                        </div>
                    </div>
        ...
        <div class="text-center text-xs text-gray-500 dark:text-gray-400">
            <span class="inline-flex items-center gap-4">
                <span><kbd class="px-2 py-1 bg-gray-200 dark:bg-gray-700 rounded">Space</kbd> Play/Pause</span>
                <span><kbd class="px-2 py-1 bg-gray-200 dark:bg-gray-700 rounded">←</kbd> -10s</span>
                <span><kbd class="px-2 py-1 bg-gray-200 dark:bg-gray-700 rounded">→</kbd> +10s</span>
                <span><kbd class="px-2 py-1 bg-gray-200 dark:bg-gray-700 rounded">M</kbd> Mute</span>
            </span>
        </div>
    </div>
    
    {{-- Error State --}}
    <div x-show="!audioUrl && !loading" class="relative bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/20 p-8">
        <div class="text-center">
            ...
            <p class="text-gray-600 dark:text-gray-400">Keine Audioaufnahme verfügbar</p>
        </div>
    </div>
    
    {{-- Loading State --}}
    <div x-show="loading" class="relative bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/20 p-8">
        <div class="text-center">
            <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
            <p class="text-gray-600 dark:text-gray-400">Audio wird geladen...</p>
        </div>
    </div>
</div>
...
<script>
// Removed duplicate function definition - see corrected version below
        loading: true,
        currentTime: 0,
        actualDuration: 0,
        ...
        initWaveSurfer() {
            ...
            this.wavesurfer = WaveSurfer.create({
                ...
                cursorColor: 'rgba(59, 130, 246, 0.7)',
                barWidth: 3,
                barRadius: 3,
                responsive: true,
                height: 80,
            ...
            this.wavesurfer.on('ready', () => {
                ...
                if (this.dbDuration > 0 && Math.abs(this.actualDuration - this.dbDuration) > 2) {
                    ...
                    console.warn(`Duration mismatch detected! DB: ${this.dbDuration}s, Actual: ${this.actualDuration}s`);
                    
                    // Adjust sentiment timestamps if there's a mismatch
        ...
        updateCurrentSentence() {
            // Update current sentence from transcript
            if (this.transcriptObject.length > 0) {
                const sentence = this.transcriptObject.find(s => 
                    this.currentTime >= (s.start || 0) && 
                    this.currentTime <= (s.end || this.actualDuration)
                );
                this.currentSentence = sentence || null;
            }
            
            // Update sentiment index
            const sentimentIndex = this.sentimentData.findIndex(s => 
                this.currentTime >= s.start_time && 
                this.currentTime <= s.end_time
            );
            
            if (sentimentIndex !== this.currentSentimentIndex) {
                this.currentSentimentIndex = sentimentIndex;
                
                // Emit event for transcript sync
                window.dispatchEvent(new CustomEvent('audio-time-update', {
                    detail: {
                        currentTime: this.currentTime,
                        sentenceIndex: sentimentIndex,
                        sentence: this.sentimentData[sentimentIndex] || null
                    }
                }));
            }
        },
        ...
        setupEventListeners() {
            // Listen for sentence selection from transcript
            window.addEventListener('sentence-selected', (event) => {
                if (event.detail.startTime) {
                    this.seekToTime(parseFloat(event.detail.startTime));
                }
            });
        },
...
#waveform-{{ $callId }}::-webkit-scrollbar {
    height: 6px;
}

#waveform-{{ $callId }}::-webkit-scrollbar-track {
    background: transparent;
}

#waveform-{{ $callId }}::-webkit-scrollbar-thumb {
...
@endpush
        ...
        <div class="relative mb-6">
            ...
            {{-- Waveform Container --}}
            <div id="waveform-ultra-fixed-{{ $callId }}" x-ref="waveformContainer" 
...
@push('scripts')
<script>
function audioPlayerUltraFixed(audioUrl, sentimentData, transcriptObject, dbDuration, callId) {
    return {
        audioUrl: audioUrl,
        ...
        initializeWaveSurfer() {
            ...
            this.wavesurfer.on('ready', () => {
                ...
                this.actualDuration = this.wavesurfer.getDuration();
                this.durationMismatch = Math.abs(this.actualDuration - this.dbDuration) > 5;
                console.log('WaveSurfer ready - Actual duration:', this.actualDuration, 'DB duration:', this.dbDuration);
                this.drawSentimentTimeline();
                
                // Set initial volume
                this.changeVolume();
                
                // Adjust sentiment data timestamps if needed
                if (this.durationMismatch && this.sentimentData.length > 0) {
                    const ratio = this.actualDuration / this.dbDuration;
                    this.sentimentData = this.sentimentData.map(s => ({
                        ...s,
                        start_time: s.start_time * ratio,
                    ...
                    console.log('Adjusted sentiment timestamps by ratio:', ratio);
                }
            });
            ...
            window.addEventListener('sentence-selected', (event) => {
                if (event.detail && event.detail.startTime !== undefined) {
                    let adjustedTime = parseFloat(event.detail.startTime);
                    
                    // Adjust time if there's a duration mismatch
                    if (this.durationMismatch) {
                        const ratio = this.actualDuration / this.dbDuration;
                        adjustedTime = adjustedTime * ratio;
                    }
...
/* Custom scrollbar for waveform */
#waveform-ultra-fixed-{{ $callId }} wave {
    scrollbar-width: thin;
    scrollbar-color: rgba(139, 92, 246, 0.3) transparent;
}

#waveform-ultra-fixed-{{ $callId }} wave::-webkit-scrollbar {
    height: 6px;
}

#waveform-ultra-fixed-{{ $callId }} wave::-webkit-scrollbar-track {
    background: transparent;
}

#waveform-ultra-fixed-{{ $callId }} wave::-webkit-scrollbar-thumb {
    background: rgba(139, 92, 246, 0.3);
    border-radius: 3px;
}

#waveform-ultra-fixed-{{ $callId }} wave::-webkit-scrollbar-thumb:hover {
