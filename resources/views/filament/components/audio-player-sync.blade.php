@php
    ...
    $actualDuration = $record->duration_sec ?? 0;
@endphp

<div 
    x-data="audioPlayerSync(@js($audioUrl), @js($sentimentData), @js($transcriptObject), @js($actualDuration), @js($callId))" 
    x-init="init()"
...
>
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2 text-sm" x-show="!loading && audioUrl">
            <div class="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded-full text-gray-600 dark:text-gray-300">
                <span x-text="formatTime(currentTime)"></span> / <span x-text="formatTime(actualDuration || duration)"></span>
            </div>
        </div>
    </div>
    ...
    <div class="relative">
        ...
        {{-- Waveform --}}
        <div id="waveform-container-{{ $callId }}" x-ref="waveformContainer" 
...
@push('scripts')
<script>
function audioPlayerSync(audioUrl, sentimentData, transcriptObject, actualDuration, callId) {
    return {
        audioUrl: audioUrl,
        ...
        initializeWaveSurfer() {
            ...
            this.wavesurfer.on('ready', () => {
                this.loading = false;
                this.duration = this.actualDuration || this.wavesurfer.getDuration();
                console.log('Audio duration:', this.duration, 'Actual duration from DB:', this.actualDuration);
...
<style>
#waveform-container-{{ $callId }} {
    border-radius: 0.5rem;
    background: rgba(0, 0, 0, 0.05);
}

.dark #waveform-container-{{ $callId }} {
