<x-filament-panels::page>
    ...
    @if(!isset($transaction) || !$transaction)
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
            ...
            <span class="block sm:inline">Transaction data is not available.</span>
            <pre>{{ print_r(get_defined_vars(), true) }}</pre>
        </div>
    @else
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert">
            <strong class="font-bold">Debug Info:</strong>
            <span class="block">Transaction ID: {{ $transaction->id ?? 'N/A' }}</span>
            <span class="block">Description: {{ $transaction->description ?? 'N/A' }}</span>
            <span class="block">Amount: {{ $transaction->amount_cents ?? 'N/A' }} cents</span>
        </div>
    @endif
