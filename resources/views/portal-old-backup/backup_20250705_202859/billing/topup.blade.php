...
<div class="py-12">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <form action="{{ route('business.billing.topup.process') }}" method="POST" id="topup-form">
                    ...
                    <div class="mt-2 max-w-xs">
                        <div class="relative rounded-md shadow-sm">
                            ...
                            <input type="number" 
                                   ...
                                   onchange="updateBonusDisplay()"
                                   oninput="updateBonusDisplay()"
            ...
            <!-- Bonus Preview -->
            <div id="bonus-preview" class="hidden bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
                <div class="flex items-center mb-3">
                    ...
                    <h4 class="text-lg font-medium text-green-900">Bonus-Vorschau</h4>
                </div>
                <div class="space-y-2">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-700">Aufladebetrag:</span>
                        <span class="font-medium text-gray-900" id="preview-amount">0,00 €</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-700">Bonus (<span id="preview-percentage">0</span>%):</span>
                        <span class="font-medium text-green-600" id="preview-bonus">+ 0,00 €</span>
                    </div>
                    <div class="border-t pt-2 flex justify-between text-base font-medium">
                        <span class="text-gray-900">Gesamt Guthaben:</span>
                        <span class="text-green-600" id="preview-total">0,00 €</span>
                    </div>
                    <p class="text-xs text-gray-600 mt-2" id="preview-rule-name"></p>
                </div>
            </div>
...
<script>
    ...
    function updateBonusDisplay() {
        ...
        if (amount >= 10) {
            const { rule, bonusAmount } = calculateBonus(amount);
            ...
            if (rule && bonusAmount > 0) {
                ...
                document.getElementById('preview-percentage').textContent = rule.bonus_percentage;
                document.getElementById('preview-bonus').textContent = '+ ' + formatCurrency(bonusAmount);
                document.getElementById('preview-total').textContent = formatCurrency(amount + bonusAmount);
                document.getElementById('preview-rule-name').textContent = rule.description;
                
                // Update button text if within suggested amounts
                updateButtonBonusText(amount, bonusAmount);
            } else {
                // Hide bonus preview if no bonus applies
                bonusPreview.classList.add('hidden');
                clearButtonBonusText();
            }
        } else {
            // Hide bonus preview for amounts below minimum
            bonusPreview.classList.add('hidden');
            clearButtonBonusText();
        }
    }
    
    function updateButtonBonusText(selectedAmount, bonusAmount) {
        document.querySelectorAll('.amount-button').forEach(button => {
            const buttonAmount = parseInt(button.querySelector('.text-lg').textContent.replace(/[^\d]/g, ''));
            
            if (buttonAmount === selectedAmount && bonusAmount > 0) {
                // Add bonus indicator to this button
                let bonusIndicator = button.querySelector('.bonus-indicator');
                if (!bonusIndicator) {
                    bonusIndicator = document.createElement('div');
                    bonusIndicator.className = 'bonus-indicator text-xs text-green-600 font-medium mt-1';
                    button.appendChild(bonusIndicator);
                }
                bonusIndicator.textContent = '+ ' + formatCurrency(bonusAmount) + ' Bonus';
            } else {
                // Remove bonus indicator from other buttons
                const indicator = button.querySelector('.bonus-indicator');
                if (indicator) {
                    indicator.remove();
                }
            }
    ...
    function clearButtonBonusText() {
        document.querySelectorAll('.bonus-indicator').forEach(indicator => {
            indicator.remove();
        });
    }
