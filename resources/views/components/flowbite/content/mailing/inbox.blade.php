{{-- Flowbite Component: inbox --}}
@props(['title' => 'Tailwind CSS Mailing Inbox Page - Flowbite', 'description' => 'Get started with a premium admin dashboard interface built with Tailwind CSS and Flowbite featuring over 50 example pages of charts, calendars, kanban boards, dashboards, CRUD pages, mailing systems, and more.'])
        ...
        <table class="min-w-full table-fixed divide-y divide-gray-200">
          <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
            ...
            {{- range  (index $.Site.Data "messages") }}
              <tr class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700" @click="window.location.href='{{ "mailing/read/" | relURL }}'">
                <td class="w-4 p-4">
                  <div class="inline-flex items-center space-x-4">
                    ...
                    <svg @click.stop class="h-5 w-5 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <td class="flex items-center space-x-4 whitespace-nowrap p-4">
                  <div class="{{ if eq .read true }}font-normal text-gray-700 dark:text-gray-400{{ else }}font-semibold text-gray-900 dark:text-white{{ end }} text-base">{{ .name }}</div>
                </td>
                <td
                  class="{{ if eq .read true }} font-normal text-gray-500 dark:text-gray-400 {{ else }} font-semibold text-gray-900 dark:text-white {{ end }} max-w-sm overflow-hidden truncate p-4 text-base xl:max-w-screen-md 2xl:max-w-screen-lg">
                  {{ .text }}
                </td>
                <td class="{{ if eq .read true }}font-normal text-gray-500 dark:text-gray-400{{ else }}font-medium text-gray-900 dark:text-white{{ end }} whitespace-nowrap p-4 text-sm">{{ .time }}</td>
              </tr>
              <!-- Unread -->
            {{ end -}}
            {{< /messages.inline >}}
